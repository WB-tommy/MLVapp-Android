...
bool CacheBitmapClasses(JNIEnv *env, JniCache &cache) {
  ...
  if (!localBitmapCls) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to find android/graphics/Bitmap");
    return false;
  }
  cache.bitmapClass =
      reinterpret_cast<jclass>(env->NewGlobalRef(localBitmapCls));
  env->DeleteLocalRef(localBitmapCls);
  if (!cache.bitmapClass) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to create global ref for Bitmap");
    return false;
  }
  ...
  if (!localConfigCls) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to find Bitmap$Config");
    return false;
  }
  cache.bitmapConfigClass =
      reinterpret_cast<jclass>(env->NewGlobalRef(localConfigCls));
  env->DeleteLocalRef(localConfigCls);
  if (!cache.bitmapConfigClass) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to create global ref for Bitmap$Config");
    return false;
  }

  cache.bitmapCreateMethod = env->GetStaticMethodID(
      cache.bitmapClass, "createBitmap",
      "(IILandroid/graphics/Bitmap$Config;)Landroid/graphics/Bitmap;");
  if (!cache.bitmapCreateMethod) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to cache Bitmap.createBitmap");
    return false;
  }

  jfieldID configArgb8888Field = env->GetStaticFieldID(
      cache.bitmapConfigClass, "ARGB_8888", "Landroid/graphics/Bitmap$Config;");
  if (!configArgb8888Field) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to cache Bitmap$Config.ARGB_8888 field");
    return false;
  }

  jobject localConfig =
      env->GetStaticObjectField(cache.bitmapConfigClass, configArgb8888Field);
  if (!localConfig) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to obtain Bitmap$Config.ARGB_8888 object");
    return false;
  }
  ...
  if (!cache.bitmapConfigArgb8888) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to promote ARGB_8888 to global ref");
    return false;
  }
...
bool CacheClipClasses(JNIEnv *env, JniCache &cache) {
  jclass localPreviewCls =
      env->FindClass("fm/magiclantern/forum/data/ClipPreviewData");
  if (!localPreviewCls) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to find ClipPreviewData class");
    return false;
  }
  cache.clipPreviewDataClass =
      reinterpret_cast<jclass>(env->NewGlobalRef(localPreviewCls));
  env->DeleteLocalRef(localPreviewCls);
  if (!cache.clipPreviewDataClass) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to create global ref for ClipPreviewData");
    return false;
  }
  ...
  jclass localMetaCls =
      env->FindClass("fm/magiclantern/forum/data/ClipMetaData");
  if (!localMetaCls) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to find ClipMetaData class");
    return false;
  }
  cache.clipMetaDataClass =
      reinterpret_cast<jclass>(env->NewGlobalRef(localMetaCls));
  env->DeleteLocalRef(localMetaCls);
  if (!cache.clipMetaDataClass) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "Failed to create global ref for ClipMetaData");
    return false;
  }
...
JniCache &GetJniCache() { return g_cache; }
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
  ...
  if (!EnsureJniCacheInitialized(env)) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "JNI_OnLoad: cache initialization failed");
    DestroyJniCache(env);
    return JNI_ERR;
  }
...
JNIEXPORT void JNICALL JNI_OnUnload(JavaVM *vm, void *) {
  ...
  if (!env) {
    __android_log_print(ANDROID_LOG_WARN, kLogTag,
                        "JNI_OnUnload: unable to obtain JNIEnv");
    return;
  }
  DestroyJniCache(env);
  if (attachedHere && vm) {
