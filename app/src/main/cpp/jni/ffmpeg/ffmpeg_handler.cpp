...
int export_image_sequence(mlvObject_t *video, const export_options_t &options,
    ...
    LOGI(LOG_TAG,
         "Image sequence: %d frames, %dx%d, codec_id=%d (direct write mode)",
         total_frames, dst_w, dst_h, codec_id);

    for (int i = 0; i < total_frames; ++i) {
        if (is_export_cancelled()) {
            ret = EXPORT_CANCELLED;
            LOGI(LOG_TAG, "Image sequence export cancelled at frame %d", i);
            break;
        }
        ...
        LOGI(LOG_TAG, "Requesting frame fd: %s (fd=%d)", relative_name, fd);
        if (fd < 0) {
...
int export_video_container(mlvObject_t *video, const export_options_t &options,
    ...
    LOGI(LOG_TAG, "Export settings: codec=%d, dimensions=%dx%d, fps=%d/%d",
         preset.codec_id, dst_w, dst_h, fps.num, fps.den);
    ...
    for (; frame_idx < total_frames; ++frame_idx) {
        ...
        while (true) {
            ...
            if (enc_ret < 0) {
                ...
                LOGE(LOG_TAG, "avcodec_receive_packet failed for frame %d with error %d: %s",
